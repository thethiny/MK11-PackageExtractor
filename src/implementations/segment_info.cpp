...
void SegmentInfo::read(std::ifstream& fin)
{
    ...
    compressed_segments_count = (int)(info.segment_decompressed_size / info.max_seg_dec_size) + bool(info.segment_decompressed_size % info.max_seg_dec_size);
    uint64_t data_location = fin.tellg();
    data_location += compressed_segments_count * sizeof(CompressedSegment::Info);
